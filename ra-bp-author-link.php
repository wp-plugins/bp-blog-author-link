...
function ra_bp_filter_author($content) {
	...
	if(defined('BP_MEMBERS_SLUG')) {
		$member_url = (( 'on' == strtolower($_SERVER['HTTPS']) ) ? 'https://' : 'http://') . 
			$current_site->domain . $current_site->path . BP_MEMBERS_SLUG;
		if(defined('VHOST') && constant('VHOST') != 'yes' && version_compare( $GLOBALS['wpmu_version'], '2.8', '>') &&
			$current_site->domain == $current_blog->domain && $current_site->path == $current_blog->path) {
			$extra = '/blog';
		} else {
			$extra = '';
		}
		$blog_url = get_option('siteurl') . $extra . '/author';
		$content = str_replace($blog_url,$member_url, $content);
	}
	return $content;
}
?>